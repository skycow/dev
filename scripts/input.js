...
//
// ------------------------------------------------------------------
Rocket.input.Keyboard = function() {
	'use strict';
	let keys = {},
		keyRepeat = {},
		handlers = {},
	...
	function keyRelease(event) {
        if (event.keyCode === Rocket.input.KeyEvent.DOM_VK_DOWN && !flipFlag){
            flipFlag = true;
        }
			...
			if (handlers.hasOwnProperty(key)) {
				for (let entry = 0; entry < handlers[key].length; entry += 1) {
					...
					if (event.repeat === true) {
                        ...
                        if (event.key === Rocket.input.KeyEvent.DOM_VK_DOWN && flipFlag){
                            if (flipFlag){
                                event.handler(elapsedTime);
                            }
                            flipFlag = false;
                        } else if (event.key !== Rocket.input.KeyEvent.DOM_VK_DOWN) {
                            event.handler(elapsedTime);
                            keyRepeat[key] = true;
                        }
					}
...
//
//------------------------------------------------------------------
Rocket.input.KeyEvent = (function() {
	'use strict';
	let that = {
